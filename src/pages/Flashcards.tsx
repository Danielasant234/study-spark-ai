...
export default function Flashcards() {
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'flashcards' }, () => { loadCards(); })
      .subscribe();
  ...
  const wrongCards = filteredCards.filter(c => c.times_incorrect > 0).sort((a, b) => {
    ...
    const bR = b.times_incorrect / Math.max(1, b.times_correct + b.times_incorrect);
    return bR - aR;
  });
  ...
  const startStudy = (selectedMode: StudyMode) => {
    ...
    switch (selectedMode) {
      ...
      case 'quick': deck = [...filteredCards].sort(() => Math.random() - 0.5).slice(0, 10); break;
  ...
  const answerCard = async (quality: number) => {
    ...
    await supabase.from('flashcards').update({
      ease_factor: sm2.ease_factor, interval: sm2.interval, repetitions: sm2.repetitions,
      next_review: sm2.next_review, last_reviewed: new Date().toISOString(),
    ...
    if (currentIndex + 1 >= total) {
      ...
      await supabase.from('review_sessions').insert({
        mode, subject: subjectFilter === 'all' ? null : subjectFilter,
      ...
      setTimeout(() => { setCurrentIndex(i => i + 1); setCardStartTime(Date.now()); }, 200);
    }
  };

  const navigate = (dir: 1 | -1) => {
    setIsFlipped(false);
    setTimeout(() => { setCurrentIndex(i => (i + dir + total) % total); setCardStartTime(Date.now()); }, 200);
  };
  ...
  const subjectStats = subjects.map(s => {
    ...
    const t = cards.reduce((sum, c) => sum + c.times_correct + c.times_incorrect, 0);
    const correct = cards.reduce((sum, c) => sum + c.times_correct, 0);
    return { subject: s, cards: cards.length, accuracy: t > 0 ? Math.round((correct / t) * 100) : 0, due: cards.filter(c => isDueForReview(c.next_review)).length };
  });
    ...
    return (
      <div className="space-y-6">
        <div ref={headerRef} className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-heading text-xl sm:text-2xl font-bold tracking-tight text-foreground">Estatísticas</h1>
        ...
        <div ref={contentRef} className="space-y-6">
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
            ...
            <StatCard icon={Brain} label="Para Revisar" value={String(dueCards.length)} color="text-accent-foreground" />
          ...
          {subjectStats.length > 0 && (
            <div className="rounded-xl border border-border bg-card p-4 sm:p-5 shadow-sm">
              ...
              <div className="space-y-3">
                {subjectStats.map(s => (
                  <div key={s.subject} className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                    ...
                    {s.due > 0 && (
                      <span className="shrink-0 rounded-md bg-accent/15 px-2 py-0.5 text-xs font-medium text-accent-foreground self-start">{s.due} pendentes</span>
                    )}
                  </div>
                ))}
              </div>
    ...
    return (
      <div className="space-y-6">
        <div ref={headerRef} className="text-center">
          ...
          <h1 className="font-heading text-xl sm:text-2xl font-bold tracking-tight text-foreground">Sessão Completa! 🎉</h1>
          <p className="text-sm text-muted-foreground mt-1">{STUDY_MODES.find(m => m.id === mode)?.label}</p>
        </div>
        <div ref={contentRef} className="mx-auto max-w-md space-y-4 px-4">
          <div className="grid grid-cols-3 gap-3">
            <div className="rounded-xl border border-border bg-card p-3 sm:p-4 text-center shadow-sm">
              <p className="text-xl sm:text-2xl font-bold text-success">{sessionCorrect}</p>
              <p className="text-xs text-muted-foreground">Acertos</p>
            </div>
            <div className="rounded-xl border border-border bg-card p-3 sm:p-4 text-center shadow-sm">
              <p className="text-xl sm:text-2xl font-bold text-destructive">{sessionIncorrect}</p>
              <p className="text-xs text-muted-foreground">Erros</p>
            </div>
            <div className="rounded-xl border border-border bg-card p-3 sm:p-4 text-center shadow-sm">
              <p className="text-xl sm:text-2xl font-bold text-foreground">{accuracy}%</p>
      ...
      <div className="space-y-4 sm:space-y-6">
        <div ref={headerRef} className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 sm:gap-4 text-sm">
            <span className="flex items-center gap-1 text-success font-medium"><Check className="h-3.5 w-3.5 sm:h-4 sm:w-4" />{sessionCorrect}</span>
            <span className="flex items-center gap-1 text-destructive font-medium"><X className="h-3.5 w-3.5 sm:h-4 sm:w-4" />{sessionIncorrect}</span>
          </div>
        </div>
        ...
        <div ref={contentRef} className="flex justify-center px-2">
          <div className="perspective-1000 w-full max-w-xl cursor-pointer" onClick={flip}>
            <div className={cn("preserve-3d relative h-56 sm:h-72 w-full transition-transform duration-500", isFlipped && "rotate-y-180")}>
              <div className="backface-hidden absolute inset-0 flex items-center justify-center rounded-2xl border border-border bg-card p-6 sm:p-8 shadow-lg">
                <div className="text-center">
                  ...
                  <p className="mt-4 sm:mt-6 text-xs text-muted-foreground">Toque para virar</p>
                </div>
              </div>
              <div className="backface-hidden rotate-y-180 absolute inset-0 flex items-center justify-center rounded-2xl border border-primary/20 bg-primary/5 p-6 sm:p-8 shadow-lg">
                <div className="text-center">
                  <p className="text-xs font-medium uppercase tracking-widest text-primary">Resposta</p>
                  <p className="mt-3 sm:mt-4 text-sm sm:text-base text-foreground leading-relaxed">{card.back}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex items-center justify-center gap-2 sm:gap-3">
          <button onClick={() => answerCard(0)} title="Errei"
            className="flex h-11 w-11 sm:h-12 sm:w-12 items-center justify-center rounded-full border border-destructive/20 bg-destructive/5 text-destructive transition-all duration-200 hover:bg-destructive/10 active:scale-95">
            <X className="h-5 w-5" />
          </button>
          <button onClick={() => navigate(-1)}
            className="flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-full border border-border bg-card text-muted-foreground transition-all duration-200 hover:bg-secondary active:scale-95">
          ...
          <button onClick={() => answerCard(3)} title="Difícil"
            className="flex h-10 w-10 sm:h-11 sm:w-11 items-center justify-center rounded-full border border-accent/30 bg-accent/10 text-accent-foreground transition-all duration-200 hover:bg-accent/20 active:scale-95">
            <Flame className="h-4 w-4" />
          </button>
          <button onClick={() => answerCard(5)} title="Fácil"
            className="flex h-11 w-11 sm:h-12 sm:w-12 items-center justify-center rounded-full border border-success/20 bg-success/5 text-success transition-all duration-200 hover:bg-success/10 active:scale-95">
  ...
  return (
    <div className="space-y-6">
      <div ref={headerRef} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="font-heading text-2xl font-bold tracking-tight text-foreground">Flashcards</h1>
          <p className="text-sm text-muted-foreground">
            {allCards.length} cards · {dueCards.length} para revisar hoje
          </p>
        </div>
        <button onClick={() => setView('stats')}
          className="flex items-center justify-center gap-2 rounded-lg border border-border bg-card px-3 py-2.5 text-sm font-medium text-muted-foreground shadow-sm transition-all hover:bg-secondary hover:text-foreground active:scale-[0.97] self-start sm:self-auto">
      ...
      <div ref={contentRef} className="space-y-6">
        ...
        {allCards.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border bg-card p-8 sm:p-12 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground max-w-sm">
              Gere flashcards automaticamente na página <strong>Gerar Materiais</strong> selecionando "Flashcards".
            </p>
          </div>
        ) : (
          <>
            ...
            {dueCards.length > 0 && (
              <button onClick={() => startStudy('due')}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-muted-foreground">Revisão espaçada SM-2</p>
                </div>
